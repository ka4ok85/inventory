...
public class UserService {
    ...
    public Iterable<Userstore> getAllByStoreAndStatus(Long storeId, String status) {
        Store store = storeRepository.findOne(storeId);
        if (store == null) {
            throw new NotFoundException(storeId.toString());
        }

        if (status.equalsIgnoreCase(Userstore.STATUS_ACTIVE) && status.equalsIgnoreCase(Userstore.STATUS_DISABLED)) {
            throw new NotFoundException("Wrong user status: " + status);
        }

        return userstoreRepository.findById(store, status);
    }
/*
